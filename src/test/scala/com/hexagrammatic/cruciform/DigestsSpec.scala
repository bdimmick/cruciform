...
class DigestsSpec extends FlatSpec with Matchers {
  ...
  def assertDigest(data: String, digest: Array[Byte], alg: String = Digests.Constants.DEFAULT_DIGEST_ALGORITHM) {
    digest should not be (null)
    MessageDigest.getInstance(alg).digest(data.getBytes) should equal (digest)
  }

  def assertHMAC(data: String, key: Key, hmac: Array[Byte], alg: String = Digests.Constants.DEFAULT_HMAC_ALGORITHM) {
